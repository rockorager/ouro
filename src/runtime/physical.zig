...
pub fn Coordinator(comptime protocol: type) type {
    return struct {
        ...
        hotplug_updated_connector_ids: []u32,
        hotplug_updated_connector_count: usize = 0,
        ...
        pub fn create(
            ...
            self.hotplug_updated_connector_ids = try allocator.alloc(
                u32,
                config.drm.connector_capacity,
            );
            errdefer allocator.free(self.hotplug_updated_connector_ids);
        ...
        pub fn backendDrainComplete(self: *const Self) bool {
            return self.stopping and !self.anyKmsOutput() and
                !self.anyPendingScreencopy() and !self.anyPendingImageCopy() and
        ...
        pub fn destroy(self: *Self) !void {
            ...
            self.allocator.free(self.hotplug_updated_connector_ids);
            ...
            for (self.physical_outputs[0..self.physical_output_count]) |physical| {
                var capture_bytes = physical.capture_bytes;
        ...
        fn processHotplug(self: *Self) anyerror!void {
            ...
            const probe = try self.manager.probeConnectorChanges(
                ...
                self.hotplug_updated_connector_ids,
            );
            ...
            if ((primary_missing and !primary_promoted) or added or probe.desktop_updated.len != 0 or
        ...
        fn requestTopologyRefresh(self: *Self, updated_connectors: []const u32) !void {
            ...
            self.drm_lease_topology_generation = null;
            for (self.physical_outputs[0..self.physical_output_count]) |*physical| {
                if (std.mem.indexOfScalar(u32, updated_connectors, physical.connector_id) == null)
                    continue;
        ...
        pub fn anyPendingImageCopy(self: *const Self) bool {
            for (self.physical_outputs[0..self.physical_output_count]) |physical|
                if (physical.pending_image_copy != null) return true;
            return false;
        }
        ...
        fn globalVisible(
            ...
            return !std.mem.eql(u8, name, protocol.wp_security_context_manager_v1.info.name) and
                ...
                !std.mem.eql(u8, name, "zwlr_output_manager_v1") and
                ...
                !std.mem.eql(u8, name, "ext_session_lock_manager_v1") and
        ...
        fn sessionLockOutput(self: *Self, layer: *const Layer) ?OutputAdapter.OutputId {
            const id = layer.id orelse return null;
            if (self.session_lock_adapter.outputForSurface(id)) |output| return output;
            const scene = self.surfaceScene(id) orelse return null;
            return self.session_lock_adapter.outputForSurface(scene.root.surface);
        }
        ...
        fn addPhysicalOutput(
            ...
            const capture_bytes = if (reusable != null)
                self.physical_outputs[index].capture_bytes
            else
        ...
        fn renderFrame(self: *Self, frame: @import("../output/headless.zig").FrameId) !void {
            ...
            else if (physical.pending_image_copy) |pending|
                if (pending.awaiting_output) .{
                    .token = pending.token,
                    .cursor_start = cursor_start,
                    .overlay_cursor = pending.overlay_cursor,
        ...
        fn captured(
            ...
            for (self.physical_outputs[0..self.physical_output_count]) |*physical| {
                ...
                break;
            } else for (self.physical_outputs[0..self.physical_output_count]) |*physical| {
                const pending = physical.pending_image_copy orelse continue;
                if (token != pending.token) continue;
        ...
        fn processScreencopyCaptures(self: *Self) !void {
            ...
            physical.capture_bytes.resize(self.allocator, capture_bytes) catch {
                self.shm.store.unpin(pin) catch unreachable;
                try self.failQueuedScreencopy(capture);
                return;
            };
        ...
        fn nextReadyScreencopyCapture(self: *Self) !?ScreencopyAdapter.Capture {
            ...
            while (self.screencopy_adapter.nextCapture(after)) |candidate| {
                ...
                if (physical.pending_image_copy != null) continue;
        ...
        fn processImageCopyCaptures(self: *Self) !void {
            ...
            physical.pending_image_copy.?.awaiting_output = true;
            try self.processImageCopyCaptures();
        }
        ...
        fn nextReadyImageCopyCapture(self: *Self) ?ImageCopyCaptureAdapter.Capture {
            ...
            while (self.image_copy_capture_adapter.nextCapture(after)) |candidate| {
                ...
                const physical = self.physicalOutputForKmsId(selected.outputId()) orelse
                    return capture;
                if (physical.pending_screencopy != null or physical.pending_image_copy != null)
                    continue;
                return capture;
            }
            return null;
        ...
        fn failCapturesForOutput(
            ...
            if (self.physicalOutputForKmsIdMutable(output)) |physical| {
                ...
                if (physical.pending_image_copy != null) {
                    try self.finishImageCopy(physical, false, 0, null);
                }
            }
        }
        ...
        fn finishActiveCapture(
            ...
            if (physical.pending_image_copy != null)
                return self.finishImageCopy(physical, success, timestamp_ns, readback);
        ...
        fn finishImageCopy(
            ...
            const pending = &(physical.pending_image_copy orelse return error.MissingCapture);
            if (!pending.copied) {
            ...
            result catch |cause| switch (cause) {
                error.StaleFrame, error.InvalidCompletion => {
                    physical.pending_image_copy = null;
                    return;
                },
                else => return cause,
            };
            ...
            for (self.clients.items) |*client| if (client.active) {
                for (self.physical_outputs[0..self.physical_output_count]) |*physical| {
                    ...
                    for (self.app_layers[0..self.app_layer_count]) |*layer| {
                        ...
                        } else if (try clipToOutput(layer.sample.?.destination, bounds) == null) continue;
        ...
        fn refreshChangedTopology(self: *Self) !void {
            ...
            const handle = self.manager.rescanReplacingClaims(
            ...
            for (self.physical_outputs[0..self.physical_output_count]) |*physical| {
                ...
                physical.claim = null;
                for (try self.manager.scanoutCandidates(handle)) |candidate| {
                    if (topology.connectors[candidate.connector_index].id != physical.connector_id)
                        continue;
                    const claim = try self.manager.claimScanout(handle, candidate);
                    errdefer self.manager.releaseScanout(claim) catch {};
                    const snapshot = try self.manager.claimSnapshot(claim);
                    const state = try self.output_management_adapter.lifecycle.currentHead(
                        physical.management_head,
                    );
